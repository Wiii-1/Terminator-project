	...
	if npcHandItem:IsWeapon() == true and npcHandItem:isRanged() == true then
		...
		then
			...
			if lastAction then
				...
				if
					lastAction.Type ~= "ISEjectMagazine"
...
function PZNS_GeneralAI.PZNS_CanSeeAimTarget(npcSurvivor)
	...
	if PZNS_WorldUtils.PZNS_IsObjectZombieActive(npcSurvivor.aimTarget) == true then
		local distanceFromTarget =
			PZNS_WorldUtils.PZNS_GetDistanceBetweenTwoObjects(npcIsoPlayer, npcSurvivor.aimTarget)
		...
		for npcID, targetIsoPlayer in pairs(PZNS_CellNPCsList) do
			...
			if npcIsoPlayer:getModData().survivorID ~= npcID then
				if isNPCHostileToTargetNPC == true then
					...
					local distanceFromTargetNPC =
						PZNS_WorldUtils.PZNS_GetDistanceBetweenTwoObjects(npcIsoPlayer, targetIsoPlayer)
					--
...
function PZNS_GeneralAI.PZNS_IsFacingLockedDoor(npcSurvivor)
	...
	if door ~= nil then
		local distanceFromDoor = PZNS_WorldUtils.PZNS_GetDistanceBetweenTwoObjects(npcIsoPlayer, door)
...
function PZNS_GeneralAI.PZNS_IsFacingLockedWindow(npcSurvivor)
	...
	if window ~= nil then
		local distanceFromTarget = PZNS_WorldUtils.PZNS_GetDistanceBetweenTwoObjects(npcIsoPlayer, window)
...
function PZNS_GeneralAI.PZNS_IsNPCHostileToTargetNPC(npcSurvivor, targetNPCSurvivor)
	if
		PZNS_UtilsNPCs.IsNPCSurvivorIsoPlayerValid(npcSurvivor) == false
	...
	if
		PZNS_GeneralAI.PZNS_IsNPCFriendlyToPlayer(npcSurvivor) == true
