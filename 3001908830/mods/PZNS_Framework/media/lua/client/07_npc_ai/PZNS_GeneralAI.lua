...
-- ============================================================================
-- GOAP Terminator Brain Integration
-- ============================================================================
-- Try to require the GOAP brain safely so a missing/errored module doesn't crash OnTick
local GOAPTerminatorBrain = nil
do
    local ok, modOrErr = pcall(function() return require("07_npc_ai/PZNS_GOAPTerminatorBrain") end)
    if ok and modOrErr then
        GOAPTerminatorBrain = modOrErr
    else
        print("[PZNS_GeneralAI] Warning: failed to load PZNS_GOAPTerminatorBrain ->", tostring(modOrErr))
        GOAPTerminatorBrain = nil
    end
end

-- Hook into PZNS update cycle (defensive: check module and tick function)
Events.OnTick.Add(function()
    local activeNPCs = PZNS_UtilsDataNPCs.PZNS_GetCreateActiveNPCsModData()
    if not activeNPCs then return end

    for survivorID, npcSurvivor in pairs(activeNPCs) do
        -- Only run GOAP for Terminators
        if npcSurvivor and npcSurvivor.isTerminator then
            if GOAPTerminatorBrain and type(GOAPTerminatorBrain.tick) == "function" then
                local ok, err = pcall(GOAPTerminatorBrain.tick, npcSurvivor)
                if not ok then
                    print("[PZNS_GeneralAI] Error running GOAPTerminatorBrain.tick for ", tostring(survivorID), ": ", tostring(err))
                end
            else
                -- First time notice (avoid spamming every tick)
                if not GOAPTerminatorBrain then
                    -- Attempt to load again in case it failed previously but is now available
                    local reloadOk, modOrErr = pcall(function() return require("07_npc_ai/PZNS_GOAPTerminatorBrain") end)
                    if reloadOk and modOrErr then
                        GOAPTerminatorBrain = modOrErr
                    else
                        -- Only print once per tick loop; it's cheap but avoids nil indexing crash
                        -- print("[PZNS_GeneralAI] GOAPTerminatorBrain not available; skipping GOAP ticks")
                    end
                else
                    print("[PZNS_GeneralAI] GOAPTerminatorBrain.tick is not a function")
                end
            end
        end
    end
