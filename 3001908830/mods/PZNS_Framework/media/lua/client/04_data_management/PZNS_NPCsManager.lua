...
--- Spawn a Terminator NPC at the specified coordinates and mark it for GOAP
---@param x number
---@param y number
---@param z number
---@return PZNS_NPCSurvivor?|nil
function PZNS_NPCsManager.SpawnTerminator(x, y, z)
    local square = getCell():getGridSquare(x, y, z)
    if not square then
        print(string.format("[PZNS_NPCsManager.SpawnTerminator] Invalid square: %s,%s,%s", tostring(x), tostring(y), tostring(z)))
        return nil
    end

    local terminator = PZNS_NPCsManager.createNPCSurvivor(
        "Terminator_" .. tostring(getTimestampMs()),
        false, -- isFemale
        "Terminator",
        "The",
        square
    ...
    -- Mark as Terminator for GOAP brain
    ...
    if PZNS_UtilsNPCs.PZNS_EnsureNPCAlive then
        PZNS_UtilsNPCs.PZNS_EnsureNPCAlive(terminator)
    end

    -- Setup equipment (give a pistol and attempt to load it)
    PZNS_UtilsNPCs.PZNS_AddEquipWeaponNPCSurvivor(terminator, "Base.Pistol")
    -- Attempt to set the gun loaded if helper exists
    if PZNS_UtilsNPCs.PZNS_SetLoadedGun then
        PZNS_UtilsNPCs.PZNS_SetLoadedGun(terminator)
    end
